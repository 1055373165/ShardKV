...
## 主线描述
- shardkv 需要定时从 shard controller 拉取最新配置，然后根据配置来确定哪些 shard 需要迁移和删除，并在 Leader 节点上进行操作，最后通过配置变更通知集群中所有 Follower。（在这一点上和配置中心 paladin 的订阅监听机制是类似的，长轮询拉取配置变更信息，根据变更事件进行配置修改；区块链中定时从中央节点拉取最新的区块，原理都是类似的）
